package themapapp.crackeddish.com.themapproject;
...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback,GoogleMap.OnMarkerClickListener {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        mSydeny=mMap.addMarker(new MarkerOptions().position(SYDNEY).title("Sydeny")
                .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_YELLOW)));
        mSydeny.setTag(0);
        markerList.add(mSydeny);
        mBrisbane=mMap.addMarker(new MarkerOptions().position(BRISBANE).title("Brisbane")
                .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_CYAN)));
        mBrisbane.setTag(0);
        markerList.add(mBrisbane);
        mMap.setOnMarkerClickListener(this);//Register click listner
